...
const LABEL_SELECTION_INTERVAL = 450;
const LABEL_PIXEL_RATIO = 2;
const IDLE_ORBIT_DELAY = 1300;
const IDLE_ORBIT_SPEED = 0.018;
...
let catalogPresenceField = null;
let catalogPresenceMaterial = null;
...
function featuredAtlasCenter() {
    const center = new THREE.Vector3();
    const source = featuredTargets.length ? featuredTargets : targets;
    if (!source.length) {
        return center;
    }

    source.forEach((target) => {
        center.x += target.position[0];
        center.y += target.position[1];
        center.z += target.position[2];
    });
...
const CATALOG_ARCHETYPES = [
    {
        ...
        summary: "A stable knowledge star suspended in the wider luminous atlas."
    },
    {
        spectrum: "K2 IV",
        spectralIndex: 0.86,
...
function generateCatalogTargets(count, existing = []) {
    ...
    const clusterAnchors = existing.map((target) => targetPosition(target));
    if (clusterAnchors.length) {
        clusterAnchors.forEach((position) => coreCenter.add(position));
        coreCenter.multiplyScalar(1 / clusterAnchors.length);
    }

    const planeX = new THREE.Vector3(0.93, 0.08, -0.36).normalize();
    const planeY = new THREE.Vector3(-0.12, 0.98, 0.16).normalize();
    const planeZ = new THREE.Vector3().crossVectors(planeX, planeY).normalize();
    planeY.crossVectors(planeZ, planeX).normalize();
    const gaussian = () => {
        const u = Math.max(0.000001, random());
        const v = Math.max(0.000001, random());
        return Math.sqrt(-2 * Math.log(u)) * Math.cos(TAU * v);
    };
    const projectAtlas = (x, y, z) => {
        return [
            coreCenter.x + planeX.x * x + planeY.x * y + planeZ.x * z,
            coreCenter.y + planeX.y * x + planeY.y * y + planeZ.y * z,
            coreCenter.z + planeX.z * x + planeY.z * y + planeZ.z * z
        ];
    };
    const roleBoost = {
        anchor: 0.26,
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        if (role === "core") {
            ...
            const r = Math.pow(random(), 1.66) * 17500;
            const bend = Math.sin(theta * 3.0 + r * 0.00032) * 0.28 + gaussian() * 0.08;
            const x = Math.cos(theta + bend) * r * (0.82 + random() * 0.34);
            const z = Math.sin(theta - bend) * r * (0.78 + random() * 0.28);
            const y = gaussian() * (1650 + r * 0.22);
            position = projectAtlas(x, y, z);
            densityBias = 1 - clamp(r / 17500, 0, 1);
        } else if (role === "arm" || role === "anchor") {
            const arm = Math.floor(random() * 4);
            const baseAngle = arm * TAU / 4 + random() * 0.54;
            const r = 5200 + Math.pow(random(), 0.58) * 34500;
            const curve = baseAngle + r * 0.00019 + gaussian() * (role === "anchor" ? 0.07 : 0.18);
            const feather = gaussian() * (role === "anchor" ? 520 : 1320 + r * 0.035);
            const x = Math.cos(curve) * r + Math.cos(curve + Math.PI / 2) * feather;
            const z = Math.sin(curve) * r * (0.82 + random() * 0.22) + Math.sin(curve + Math.PI / 2) * feather;
            const y = gaussian() * (role === "anchor" ? 1800 : 2100 + r * 0.075);
            position = projectAtlas(x, y, z);
            densityBias = role === "anchor" ? 0.96 : 0.58 + (1 - clamp(r / 39700, 0, 1)) * 0.25;
        } else if (role === "cluster") {
            const anchor = clusterAnchors[Math.floor(random() * Math.max(1, clusterAnchors.length))] || coreCenter;
            const spread = 1900 + Math.pow(random(), 0.8) * 5400;
            position = [
                anchor.x + gaussian() * spread,
                anchor.y + gaussian() * spread * 0.58,
                anchor.z + gaussian() * spread
            ];
            densityBias = 0.74;
        } else {
            const theta = random() * TAU;
            const phi = Math.acos(2 * random() - 1);
            const shell = 10500 + Math.pow(random(), 0.54) * 38000;
            position = [
                coreCenter.x + Math.sin(phi) * Math.cos(theta) * shell,
                coreCenter.y + Math.cos(phi) * shell * 0.88 + gaussian() * 900,
                coreCenter.z + Math.sin(phi) * Math.sin(theta) * shell
            ];
            densityBias = 0.12 + Math.pow(random(), 3) * 0.18;
        }

        const radius = mix(archetype.radius[0], archetype.radius[1], random()) * (role === "anchor" ? 1.16 : role === "core" ? 1.06 : 1);
        const brightness = clamp(
            mix(archetype.brightness[0], archetype.brightness[1], Math.pow(random(), 0.62)) +
                roleBoost[role] +
                densityBias * 0.1 +
                Math.pow(random(), 8.0) * 0.12,
            0.28,
            role === "anchor" ? 1.22 : 1.08
        );
        ...
        generated.push({
            ...
            seed: hashString(id),
            atlasRole: role,
            atlasDensity: densityBias,
            atlasLabelPriority: (role === "anchor" ? 46 : role === "core" ? 18 : role === "cluster" ? 12 : 0) + brightness * 10,
            featured: false
        });
    }
...
function catalogImportance(target) {
    const roleBoost = target.atlasRole === "anchor"
        ? 90
        : target.atlasRole === "core"
            ? 44
            : target.atlasRole === "cluster"
                ? 28
                : target.atlasRole === "arm"
                    ? 14
                    : 0;
...
function labelSelectionSignature(maxLabels) {
    const anchor = mapCenterSectorVector();
    const sectorKey = skySectorKeyFromVector(anchor.x, anchor.y, anchor.z);
    return `${activeRegime}:${adaptiveLevel}:${activeTarget?.id || ""}:${sectorKey}:${maxLabels}`;
}
...
function updateCatalogVisibility(force = false) {
    ...
    const presenceColors = catalogPresenceField?.geometry?.getAttribute("color") || null;
    targets.forEach((target, index) => {
        const base = target.catalogBaseColor || [0, 0, 0];
        const presenceBase = target.catalogPresenceBaseColor || base;
        ...
        const anchor = target.atlasRole === "anchor";
        const core = target.atlasRole === "core";
        const intensity = visible
            ? featured
                ? activeRegime === "Deep Field" ? 2.18 : 1.78
                : activeRegime === "Deep Field"
                    ? anchor ? 1.82 : core ? 1.52 : 1.24
                    : activeRegime === "Cluster"
                        ? anchor ? 1.94 : core ? 1.62 : 1.46
                        : anchor ? 1.22 : 0.9
            : 0;
        const semanticBoost = active ? 2.05 : hover ? 1.72 : 1;
        colors.setXYZ(index, base[0] * intensity * semanticBoost, base[1] * intensity * semanticBoost, base[2] * intensity * semanticBoost);
        if (presenceColors) {
            const presenceIntensity = visible
                ? (active ? 1.86 : hover ? 1.48 : featured ? 1.18 : anchor ? 0.92 : core ? 0.64 : activeRegime === "Cluster" ? 0.52 : 0.34)
                : 0;
            presenceColors.setXYZ(
                index,
                presenceBase[0] * presenceIntensity,
                presenceBase[1] * presenceIntensity,
                presenceBase[2] * presenceIntensity
            );
        }
    });
    colors.needsUpdate = true;
    if (presenceColors) {
        presenceColors.needsUpdate = true;
    }
...
function createStarField(count, radius, disk = false) {
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        const brightness = disk ? 0.22 + random() * 0.42 : 0.055 + Math.pow(random(), 3.6) * 0.46;
...
function createStarClusterField(count, radius) {
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        const brightness = 0.038 + core * 0.18 + Math.pow(random(), 5.2) * 0.24;
        positions[offset] = x;
        positions[offset + 1] = y;
        positions[offset + 2] = z;
...
function updateGalaxyBackdrop(distance, dt, profile = LOD_SCENE_PROFILES.Cluster) {
    ...
    const backdropAlpha = mix(0.12, 0.64, deepAlpha) * surfaceCut * profile.backdrop;
...
function createAtlasGrid() {
    const positions = [];
    const gridRadius = 26000;
    const segments = 160;
    const pushLine = (a, b) => {
        positions.push(a[0], a[1], a[2], b[0], b[1], b[2]);
    };
    const pointOnSphere = (longitude, latitude, radius = gridRadius) => {
    ...
    [-60, -30, 0, 30, 60].forEach((degrees) => {
        ...
        for (let segment = 0; segment < segments; segment += 1) {
            const a = pointOnSphere(segment / segments * TAU, latitude);
            const b = pointOnSphere((segment + 1) / segments * TAU, latitude);
            pushLine(a, b);
        }
    });

    for (let longitudeIndex = 0; longitudeIndex < 12; longitudeIndex += 1) {
        const longitude = longitudeIndex / 12 * TAU;
        for (let segment = 0; segment < segments * 0.5; segment += 1) {
            const latitudeA = THREE.MathUtils.degToRad(-72 + segment / (segments * 0.5) * 144);
            const latitudeB = THREE.MathUtils.degToRad(-72 + (segment + 1) / (segments * 0.5) * 144);
            pushLine(pointOnSphere(longitude, latitudeA), pointOnSphere(longitude, latitudeB));
        }
    }

    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
    atlasGridMaterial = new THREE.LineBasicMaterial({
        color: 0x6d8cff,
        transparent: true,
        opacity: 0,
        blending: THREE.AdditiveBlending,
    ...
    atlasGrid = new THREE.LineSegments(geometry, atlasGridMaterial);
    atlasGrid.renderOrder = 6.35;
    atlasGrid.frustumCulled = false;
    atlasGrid.visible = false;
    scene.add(atlasGrid);
}

function updateAtlasGrid(distance, dt, profile = LOD_SCENE_PROFILES.Cluster, visualBudget = 1) {
    if (!atlasGrid || !atlasGridMaterial) {
        return;
    }

    const deepFade = smoothstep(680, 1400, distance);
    const farCut = 1 - smoothstep(5600, 9000, distance);
    const surfaceCut = smoothstep(180, 520, distance);
    const gridAlpha = clamp(deepFade * farCut * surfaceCut * profile.catalogTargets * visualBudget * 0.036, 0, 0.052);
    atlasGrid.position.copy(focusCurrent);
    atlasGrid.rotation.set(
        wrapSignedRadians(cameraState.pitch) * 0.12,
        cameraState.yaw * 0.18,
        0
    );
    atlasGridMaterial.opacity = gridAlpha;
    atlasGrid.visible = gridAlpha > 0.006;
}
...
function createBrightStarSprites(count, radius) {
    ...
    const seen = new Set();
    const sourceTargets = [...featuredTargets, ...deepCatalogTargets]
        .filter((target) => {
    ...
    for (let i = 0; i < count; i += 1) {
        const target = sourceTargets[i];
        if (!target) {
            break;
        }
        const tint = Number.parseInt(paletteColor(target).replace("#", ""), 16) || 0xffffff;
        const material = new THREE.SpriteMaterial({
            map: textureObject,
            color: tint,
            transparent: true,
        ...
        const sprite = new THREE.Sprite(material);
        sprite.position.set(target.position[0], target.position[1], target.position[2]);
        const featured = Boolean(target.featured);
        const roleScale = target.atlasRole === "anchor" ? 1.32 : target.atlasRole === "core" ? 1.12 : 1;
        const scale = (featured ? 1220 : 520 + Math.pow(target.brightness || 0.6, 2) * 720) * roleScale;
        ...
        brightStars.push({
            ...
            sprite,
            material,
            baseOpacity: featured ? 0.46 : clamp(0.12 + (target.brightness || 0.5) * 0.2 + (target.atlasRole === "anchor" ? 0.12 : 0), 0.1, 0.38),
            phase: random() * TAU
        });
    }
}
...
function createCatalogField(catalog) {
    ...
    catalog.forEach((target, index) => {
        ...
        const semanticBrightness = Math.max(0.52, target.brightness || 0.62);
        const roleGain = target.atlasRole === "anchor"
            ? 1.54
            : target.atlasRole === "core"
                ? 1.22
                : target.atlasRole === "cluster"
                    ? 1.16
                    : 1;
        const brightness = (target.featured ? 1.82 : 1.16 * roleGain) * semanticBrightness;
        const baseColor = [
            (r / 255) * brightness,
            (g / 255) * brightness,
            (b / 255) * brightness
        ...
        target.catalogPresenceBaseColor = [
            Math.min(2.4, baseColor[0] * (target.featured ? 1.48 : target.atlasRole === "anchor" ? 1.34 : 1.12) + 0.08),
            Math.min(2.4, baseColor[1] * (target.featured ? 1.44 : target.atlasRole === "anchor" ? 1.28 : 1.1) + 0.075),
            Math.min(2.4, baseColor[2] * (target.featured ? 1.38 : target.atlasRole === "anchor" ? 1.22 : 1.08) + 0.065)
        ];
    });
...
function createCatalogPresenceField(catalog) {
    const positions = new Float32Array(catalog.length * 3);
    const colors = new Float32Array(catalog.length * 3);
...
function makeLabelSlot(width = 420, height = 64, pixelRatio = LABEL_PIXEL_RATIO) {
...
function labelFontPx(target, active = false, hover = false) {
    if (active) {
        return 21;
    }
...
function drawLabel(slot, target, active = false, hover = false) {
    ...
    const featured = Boolean(target.featured);
    const fontSize = labelFontPx(target, active, hover);
    const pixelRatio = slot.pixelRatio || 1;
    const text = target.name;
    context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
    context.clearRect(0, 0, slot.width, slot.height);
    context.font = `${active || hover ? 620 : featured ? 540 : 460} ${fontSize}px "Helvetica Neue", Arial, sans-serif`;
    context.textBaseline = "middle";
    context.shadowBlur = 0;
    context.strokeStyle = "rgba(0, 0, 0, 0.66)";
    context.lineWidth = active ? 2.6 : hover ? 2.2 : 1.7;
    context.fillStyle = active ? "rgba(255, 249, 236, 0.94)" : hover ? "rgba(238, 246, 255, 0.9)" : featured ? "rgba(210, 225, 255, 0.64)" : "rgba(174, 193, 228, 0.44)";
    context.strokeText(text, 8, slot.height / 2);
    context.fillText(text, 8, slot.height / 2);
...
function labelSelectionIsLocked() {
    const distanceDelta = Math.abs(cameraState.targetDistance - cameraState.distance);
    return dragging || flightState.active || distanceDelta > Math.max(6, cameraState.distance * 0.006);
}

function makeLabelCandidate(target) {
    ...
    const hover = target.id === hoverTargetId;
    const featured = Boolean(target.featured);
    const highScoringCatalog = !featured && (
        activeRegime === "Cluster" ||
        (activeRegime === "Deep Field" && (target.brightness || 0) > 0.74) ||
        (activeRegime === "System Approach" && (target.brightness || 0) > 0.82)
    );
    ...
    const fontPx = labelFontPx(target, active, hover);
    const halfWidth = Math.min(128, Math.max(38, target.name.length * fontPx * 0.28 + 16));
    const halfHeight = fontPx * 0.74;
    ...
    return {
        ...
        score: (active ? 1000 : 0) +
            (hover ? 900 : 0) +
            (featured ? 120 : 0) +
            (target.brightness || 0.5) * 24 +
            distanceScore * 8
    };
}

function labelEntryStillInView(entry) {
    if (!entry?.target) {
        return false;
    }

    setVectorFromTarget(labelVector, entry.target);
    labelVector.project(camera);
    const x = (labelVector.x * 0.5 + 0.5) * canvasSize.width;
    const y = (-labelVector.y * 0.5 + 0.5) * canvasSize.height;
    return labelVector.z > -1 &&
        labelVector.z < 1 &&
        x > -140 &&
        x < canvasSize.width + 140 &&
        y > -80 &&
        y < canvasSize.height + 80;
}

function sceneLabelTargetsStillUseful(maxLabels) {
    if (!sceneLabelTargets.length) {
        return false;
    }

    const visibleCount = sceneLabelTargets.filter(labelEntryStillInView).length;
    const required = Math.min(sceneLabelTargets.length, Math.max(2, Math.floor(maxLabels * 0.5)));
...
function selectSceneLabelTargets(force = false) {
    ...
    if (!force && sceneLabelTargets.length && labelSelectionIsLocked()) {
        return sceneLabelTargets;
    }
    if (!force && signature === lastLabelSelectionSignature && sceneLabelTargetsStillUseful(maxLabels)) {
        return sceneLabelTargets;
    }
    if (!force && now - lastLabelSelection < LABEL_SELECTION_INTERVAL && sceneLabelTargetsStillUseful(maxLabels)) {
        return sceneLabelTargets;
    }
    ...
    currentInteractiveTargets().forEach((target) => {
        const candidate = makeLabelCandidate(target);
        if (!candidate) {
            return;
        }
...
function updateSceneLabels(force = false, budget = 1) {
    ...
    const slotBudget = Math.max(1, Math.round(sceneLabels.length * budget));
    let selected = selectSceneLabelTargets(force).slice(0, slotBudget);
    const displayPriority = [];
    const activeEntry = activeTarget ? makeLabelCandidate(activeTarget) : null;
    const hoverTarget = hoverTargetId ? currentInteractiveTargets().find((target) => target.id === hoverTargetId) : null;
    const hoverEntry = hoverTarget ? makeLabelCandidate(hoverTarget) : null;
    [activeEntry, hoverEntry].forEach((entry) => {
        if (entry && !displayPriority.some((item) => item.target.id === entry.target.id)) {
            displayPriority.push(entry);
        }
    });
    selected = [
        ...displayPriority,
        ...selected.filter((entry) => !displayPriority.some((priority) => priority.target.id === entry.target.id))
    ].slice(0, slotBudget);
    ...
    sceneLabels.forEach((slot, index) => {
        ...
        const fontPx = labelFontPx(target, active, hover);
        const labelWidth = Math.min(230, Math.max(52, target.name.length * fontPx * 0.6 + 24));
        const labelHeight = fontPx * 1.38;
        const labelGap = active || hover ? 10 : target.featured ? 9 : 8;
        ...
        labelHitTargets.push({
            ...
            rect: {
                left: labelLeft,
                right: labelRight,
                top: screenY - labelHeight * 0.5 - 10,
                bottom: screenY + labelHeight * 0.5 + 10
            }
        });
        ...
        const offset = unitPerPixel * labelGap;
        ...
        const sizeFactor = active ? 1 : hover ? 0.98 : target.featured ? 0.95 : 0.92;
        slot.sprite.scale.set(unitPerPixel * slot.width * sizeFactor, unitPerPixel * slot.height * sizeFactor, 1);
        slot.material.opacity += ((active ? 0.94 : hover ? 0.88 : target.featured ? 0.54 : 0.34) - slot.material.opacity) * 0.32;
...
function updateCamera(dt) {
    const idleMs = performance.now() - lastUserInteraction;
    const idleDistance = Math.max(cameraState.distance, cameraState.targetDistance);
    const idleAlpha = (!dragging && !flightState.active)
        ? smoothstep(IDLE_ORBIT_DELAY * 0.72, IDLE_ORBIT_DELAY * 1.8, idleMs) * smoothstep(1800, 9200, idleDistance)
        : 0;
    if (idleAlpha > 0.001) {
        cameraState.targetYaw += dt * IDLE_ORBIT_SPEED * idleAlpha;
    }
    ...
    if (Math.abs(cameraState.yaw) > TAU * 4) {
        const wraps = Math.trunc(cameraState.yaw / TAU);
        cameraState.yaw -= wraps * TAU;
        cameraState.targetYaw -= wraps * TAU;
    }
    ...
    // Keep the orbit continuous through the poles. A fixed world-up spherical
    // camera creates a hard singularity at the top/bottom; this mirrors the
    // 100,000 Stars style of rotating the atlas itself with unbounded angles.
    yawQuaternion.setFromAxisAngle(worldUp, cameraState.yaw);
    cameraDirection.set(0, 0, 1).applyQuaternion(yawQuaternion);
    cameraRight.set(1, 0, 0).applyQuaternion(yawQuaternion).normalize();
    pitchQuaternion.setFromAxisAngle(cameraRight, -cameraState.pitch);
    cameraDirection.applyQuaternion(pitchQuaternion).normalize();
    cameraUpVector.copy(worldUp).applyQuaternion(pitchQuaternion).normalize();
    ...
    cameraRight.crossVectors(cameraLookDirection, cameraUpVector);
...
function updateLod(normalized, dt) {
    ...
    updateAtlasGrid(distance, dt, profile, visualBudget);
    ...
    if (catalogMaterial && catalogField) {
        ...
        catalogMaterial.opacity = clamp(catalogAlpha * mix(0.9, 1.32, smoothstep(260, 4200, distance)) * visibleDensity, 0, 1);
        catalogMaterial.size = mix(2.55, 5.8, smoothstep(520, 5600, distance));
        catalogField.visible = catalogMaterial.opacity > 0.012;
    }

    if (catalogPresenceMaterial && catalogPresenceField) {
        const presenceDensity = visibleCatalogCount > 0 ? clamp(visibleCatalogCount / 280, 0.36, 1) : 0;
        const presenceBand = smoothstep(58, 420, distance) * (1 - smoothstep(9800, 21000, distance) * 0.18);
        catalogPresenceMaterial.opacity = clamp(catalogAlpha * presenceBand * presenceDensity * mix(0.36, 0.78, smoothstep(420, 4200, distance)), 0, 0.82);
        catalogPresenceMaterial.size = mix(7.2, 14.8, smoothstep(500, 6800, distance));
        catalogPresenceField.visible = catalogPresenceMaterial.opacity > 0.01;
    }
    ...
    featuredTargets.forEach((target) => {
        ...
        const spriteScale = clamp(
            angularScale * (distance > 2500 ? 0.016 : 0.034) * Math.sqrt(target.radius),
            10,
            distance > 2500 ? 230 : 340
        );

        sprite.visible = !isActive || starAlpha < 0.96;
        sprite.scale.setScalar(spriteScale * (isHover ? 1.18 : 1));
        material.opacity = clamp(baseOpacity * (isActive ? 1.16 : 1.04) + hoverBoost, 0, 1);
...
function pickTarget(x, y) {
    ...
    currentInteractiveTargets().forEach((target) => {
        ...
        const radius = target.id === activeTarget.id
            ? 92
            : target.featured
                ? (activeRegime === "Deep Field" || activeRegime === "Cluster" ? 68 : 56)
                : (activeRegime === "Deep Field" || activeRegime === "Cluster"
                    ? 34 + clamp((target.brightness || 0.5) * 30, 0, 30)
                    : 28 + clamp((target.brightness || 0.5) * 24, 0, 24));
...
async function init(message) {
    ...
    postBootProgress("minimal", 0.06, "Starting WebGPU luminous atlas");
    ...
    slowFrameWindows = 0;
    fastFrameWindows = 0;
    lastUserInteraction = performance.now() - IDLE_ORBIT_DELAY;
    ...
    activeRegime = getRegime(cameraState.distance);
    focusTarget.copy(cameraState.distance > 2500 ? featuredAtlasCenter() : targetPosition(activeTarget));
    focusCurrent.copy(focusTarget);
    ...
    catalogPresenceMaterial = new THREE.PointsMaterial({
        size: 4.8,
        sizeAttenuation: false,
        vertexColors: true,
        transparent: true,
    ...
    catalogPresenceField = new THREE.Points(createCatalogPresenceField(targets), catalogPresenceMaterial);
    catalogPresenceField.renderOrder = 6.8;
    catalogPresenceField.frustumCulled = false;
    scene.add(catalogPresenceField);
...
self.addEventListener("message", async (event) => {
    ...
    try {
        ...
        self.postMessage({
            type: "error",
            message: error.message || "Luminous stellar atlas worker error.",
