export const LUMINOUS_STELLAR_MIN_DISTANCE = 24;
export const LUMINOUS_STELLAR_MAX_DISTANCE = 40000;
export const SCALE_OBSERVATORY_MIN_DISTANCE = LUMINOUS_STELLAR_MIN_DISTANCE;
export const SCALE_OBSERVATORY_MAX_DISTANCE = LUMINOUS_STELLAR_MAX_DISTANCE;

export const LUMINOUS_STELLAR_TARGETS = [
    {
        id: "helios",
        name: "Helios Archive",
        spectrum: "G2 V",
...
export const SCALE_TARGETS = LUMINOUS_STELLAR_TARGETS;

const HUD_UPDATE_INTERVAL = 1000 / 12;
const WORKER_CACHE_VERSION = "luminous-stellar-atlas-20260428j";
const DPR_CAPS = {
    safe: 1,
    standard: 1.35,
    immersive: 1.7
...
function updateLoading(root, { progress = 0.08, detail = "Preparing luminous atlas" } = {}) {
...
function markInteraction(root, { scale = false } = {}) {
    ...
    if (root.__luminousAtlasScaleTimer) {
        clearTimeout(root.__luminousAtlasScaleTimer);
    }
    root.__luminousAtlasScaleTimer = setTimeout(() => {
        root.classList.remove("is-scale-active");
        root.__luminousAtlasScaleTimer = null;
    }, 900);
}
...
export function bootLuminousStellarAtlas({
    canvas,
    hudRoot = document,
    ...
    if (!canvas) {
        throw new Error("bootLuminousStellarAtlas requires a canvas.");
    }
    ...
    const workerUrl = new URL("./luminous-stellar-atlas-worker.js", import.meta.url);
    ...
    const pushHudState = (state, force = false) => {
        ...
        updateTargetHud(hudRoot, LUMINOUS_STELLAR_TARGETS, lastState);
    };
    ...
    worker.addEventListener("error", (error) => {
        ...
        setUnsupported(hudRoot, details);
            console.error("[luminous-stellar-atlas] worker module error", {
    ...
    worker.addEventListener("messageerror", (error) => {
        ...
        console.error("[luminous-stellar-atlas] worker message error", error);
    });
    ...
    worker.postMessage({
        ...
        budget,
        catalogCount,
        targets: LUMINOUS_STELLAR_TARGETS,
        initialTargetId,
    ...
    return {
        ...
        dispose() {
            if (hudRoot.__luminousAtlasScaleTimer) {
                clearTimeout(hudRoot.__luminousAtlasScaleTimer);
                hudRoot.__luminousAtlasScaleTimer = null;
            }
            disposers.forEach((dispose) => dispose());
