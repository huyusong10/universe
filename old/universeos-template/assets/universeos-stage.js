...
const PRESETS = {
    ...
    atlas: {
        ...
        profile: "blue",
        mainBodyVisible: false,
        star: { x: 0, y: 0, scale: 1.0 },
        mobileStar: { x: 0, y: 0, scale: 0.84 },
        focus: { x: -0.24, y: -0.18 },
        mobileFocus: { x: -0.08, y: -0.12 },
        camera: { x: 0.02, y: 0.06, z: 15.4, fov: 31.4 },
        mobileCamera: { x: 0.02, y: 0.06, z: 15.8, fov: 32.2 },
        stars: { far: 2600, near: 420 },
        fieldDensity: 1.08,
        opticsScale: 1.02,
        heroCount: 6,
        ...
        dust: [0.68, 0.5],
        accentOrbs: [
            { x: -5.8, y: 2.9, z: -50, scale: 0.24, color: "cool", haloColor: "opticCool", opacity: 0.42, haloOpacity: 0.04, twinkle: 0.05 },
            { x: -3.1, y: -1.9, z: -42, scale: 0.3, color: "warm", haloColor: "opticWarm", opacity: 0.46, haloOpacity: 0.05, twinkle: 0.07 },
            { x: 2.5, y: 1.7, z: -38, scale: 0.38, color: "cool", haloColor: "heroCool", opacity: 0.6, haloOpacity: 0.08, twinkle: 0.08 },
            { x: 5.4, y: -2.4, z: -46, scale: 0.28, color: "warm", haloColor: "opticWarm", opacity: 0.5, haloOpacity: 0.05, twinkle: 0.06 }
        ]
    },
    signalLog: {
        scene: "deepField",
        profile: "warm",
        mainBodyVisible: false,
        star: { x: 0, y: 0, scale: 0.94 },
        mobileStar: { x: 0, y: 0, scale: 0.84 },
        focus: { x: 0.78, y: -0.04 },
        mobileFocus: { x: 0.18, y: -0.1 },
        camera: { x: 0.04, y: 0.08, z: 14.8, fov: 30.6 },
        mobileCamera: { x: 0.02, y: 0.08, z: 15.2, fov: 31.2 },
        stars: { far: 2300, near: 320 },
        fieldDensity: 0.92,
        opticsScale: 0.84,
        heroCount: 5,
        glow: 0.1,
        ...
        dust: [0.48, 0.36],
        accentOrbs: [
            { x: -5.6, y: 2.4, z: -48, scale: 0.26, color: "cool", haloColor: "opticCool", opacity: 0.4, haloOpacity: 0.04, twinkle: 0.05 },
            { x: -1.8, y: 1.1, z: -32, scale: 0.52, color: "warm", haloColor: "heroWarm", opacity: 0.7, haloOpacity: 0.1, twinkle: 0.12 },
            { x: 2.2, y: -0.6, z: -34, scale: 0.34, color: "cool", haloColor: "heroCool", opacity: 0.62, haloOpacity: 0.08, twinkle: 0.1 },
            { x: 5.1, y: -2.3, z: -44, scale: 0.28, color: "warm", haloColor: "opticWarm", opacity: 0.48, haloOpacity: 0.05, twinkle: 0.06 }
        ]
    },
    cosmic: {
        profile: "warm",
    ...
    scaleLens: {
        profile: "warm",
        discOverlay: true,
        star: { x: 0.08, y: 0.06, scale: 1.12 },
        mobileStar: { x: 0.0, y: -0.16, scale: 0.92 },
        focus: { x: 0.08, y: 0.06 },
        mobileFocus: { x: 0.0, y: -0.1 },
        camera: { x: 0.0, y: 0.05, z: 18.6, fov: 33.2 },
        mobileCamera: { x: 0.0, y: 0.08, z: 19.4, fov: 34.4 },
        stars: { far: 3800, near: 720 },
        fieldDensity: 1.42,
        opticsScale: 1.42,
        heroCount: 12,
        glow: 0.17,
...
function buildStarDiscTexture(size, profile) {
    const output = textureCanvas(size);
    const context = output.getContext("2d");
    const center = size / 2;
    const radius = size * 0.46;
    const tint = profile === "blue"
        ? ["244, 252, 255", "188, 226, 255", "76, 148, 255", "8, 26, 54"]
        : ["255, 249, 232", "255, 215, 146", "255, 142, 58", "72, 20, 6"];

    const base = context.createRadialGradient(center * 0.86, center * 0.82, radius * 0.06, center, center, radius);
    base.addColorStop(0, `rgba(${tint[0]}, 1)`);
    base.addColorStop(0.34, `rgba(${tint[1]}, 0.94)`);
    base.addColorStop(0.76, `rgba(${tint[2]}, 0.72)`);
    base.addColorStop(1, `rgba(${tint[3]}, 0)`);
    ...
    for (let index = 0; index < 900; index += 1) {
        ...
        const dist = Math.sqrt(Math.random()) * radius * 0.96;
        const x = center + Math.cos(angle) * dist;
        const y = center + Math.sin(angle) * dist;
        const local = dist / radius;
        const alpha = (0.02 + Math.random() * 0.07) * (1 - local * 0.42);
        context.fillStyle = `rgba(${tint[3]}, ${alpha})`;
        context.beginPath();
        context.ellipse(x, y, 1.2 + Math.random() * 4.8, 0.7 + Math.random() * 2.8, angle, 0, TAU);
        context.fill();
    }
    ...
    for (let index = 0; index < 160; index += 1) {
        ...
        context.fillStyle = `rgba(${tint[0]}, ${alpha})`;
        context.beginPath();
        context.ellipse(x, y, 1 + Math.random() * 7, 0.8 + Math.random() * 3, angle, 0, TAU);
        context.fill();
    }
    ...
    edge.addColorStop(0.86, "rgba(255,255,255,0.94)");
...
export async function bootObservatoryStage({
    ...
    try {
        ...
        const starDiscTexture = config.discOverlay
            ? configureTexture(new THREE.CanvasTexture(buildStarDiscTexture(window.innerWidth < 900 ? 768 : 1024, config.profile)))
            : null;
        ...
        let surfaceDiscMaterial = null;
        let surfaceDiscSprite = null;
        ...
        if (mainBodyVisible) {
            ...
            if (starDiscTexture) {
                surfaceDiscMaterial = new THREE.SpriteMaterial({
                    map: starDiscTexture,
                    color: 0xffffff,
                    transparent: true,
                    opacity: 0.92,
                ...
                surfaceDiscSprite = new THREE.Sprite(surfaceDiscMaterial);
                surfaceDiscSprite.position.z = 0.16;
                surfaceDiscSprite.scale.set(3.28, 3.28, 1);
                starRoot.add(surfaceDiscSprite);
            }
        ...
        function applyLayout() {
            ...
            focusPosition.set(layoutFocus.x + rigState.focusX, layoutFocus.y + rigState.focusY, 0);
            starRoot.visible = mainBodyVisible;
            starRoot.position.set(
                mainBodyVisible ? star.x + rigState.starX : focusPosition.x,
                mainBodyVisible ? star.y + rigState.starY : focusPosition.y,
                0
            );
            starRoot.scale.setScalar(star.scale * rigState.starScale);

            baseCameraPosition.set(
                activeCamera.x + rigState.cameraX,
                activeCamera.y + rigState.cameraY,
                activeCamera.z * rigState.cameraZ
            );
            camera.fov = activeCamera.fov * rigState.cameraFov;
            renderer.toneMappingExposure = baseExposure * rigState.exposure;
        ...
        function render() {
            ...
            const isMobile = window.innerWidth < 900;
            const star = isMobile ? config.mobileStar : config.star;
            const riggedStarX = star.x + rigState.starX;
            const riggedStarY = star.y + rigState.starY;
            ...
            if (mainBodyVisible) {
                starRoot.position.x = riggedStarX + focusDriftX;
                starRoot.position.y = riggedStarY + focusDriftY;
                ...
                if (surfaceDiscMaterial) {
                    surfaceDiscMaterial.opacity = 1.02 + Math.sin(elapsed * 0.16) * 0.025 * motionFactor;
                    surfaceDiscMaterial.rotation = elapsed * 0.012 * motionFactor;
                }
        ...
        function setRig(nextState = {}) {
            Object.assign(rigState, nextState);
            applyLayout();
            render();
        }
