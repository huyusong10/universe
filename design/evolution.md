...
| 不变量 | 约束 |
| --- | --- |
| Catalog 连续性 | 宏观亮点、中景目标、近景恒星、标签和点击热区必须来自同一 catalog 索引。 |
| LOD 连续性 | 缩放时允许曝光、尺寸、光晕、参考层和标签权重变化，不允许整批亮点消失后换另一批星。 |
...
| Regional | 近邻星有强 halo、距离线、命名注记和 Oort Cloud 等桥接元素，帮助用户理解尺度变化。 | `Galactic` 辅助层退场后，`Local` 近景尚未接管，中间容易变成黑底地图和文字。 |
| Local | 近景恒星有表面纹理、corona、flare、lens flare 和持续扰动。 | 当前近景圆盘更像 2D 渐变球，动态纹理和边缘活性不足。 |
...
- 把近景恒星从径向渐变升级为动态表面：低频等离子纹理、亮斑、边缘 rim、corona 和少量 flare。
- 近景仍保留星图上下文：背景被压暗但不完全消失，邻近恒星和参考弧保持低亮。
...
| 1. Capture | 建立 LOD 视觉检查基线。 | 能固定种子抓取 Galactic、Regional、Local、飞抵过程和 zoom-out 画面；截图或视频不依赖人工临时操作。 |
...
| 4. Local Star | 升级 active 恒星近景表面和 corona。 | Local 目标读成高能恒星，而不是 UI 渐变圆盘；背景上下文仍存在。 |
...
| 验证类型 | 覆盖内容 |
| --- | --- |
| 固定种子视觉截图 | Galactic、Regional、Local、点击飞抵、zoom-out 返航。 |
| 动态录制或帧采样 | 缩放过程是否连续，曝光和标签是否跳变。 |
| Debug 状态检查 | LOD、catalog、draw、labels、fps 和 active 目标是否在预算内。 |
